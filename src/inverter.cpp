...
bool onPIGS() //QPIGS<cr>: Device general status parameters inquiry
{
  ...
  else
  {
    ...
    int index = 1; //after the starting '('
      ...
      _qpigsMessage.isswitchon = getNextBit(_commandBuffer, index);
      _qpigsMessage.isdustproof = getNextBit(_commandBuffer, index);
...
bool onMOD() //QMOD<cr>: Device Mode inquiry
{
  ...
  else
  {
    ...
    case 'C':
      ...
      break;
    case 'H':
      _qmodMessage.operationMode = "Power Save";
      break;
    }
    return true;
  }
...
String stringcomm(String cmmd){
  unsigned short crc = cal_crc_half((byte *)cmmd.c_str(), cmmd.length());

  Serial.print(cmmd);
  Serial.print((char)((crc >> 8) & 0xFF)); //ONLY CRC fo PCM/PIP
  Serial.print((char)((crc >> 0) & 0xFF)); //ONLY CRC fo PCM/PIP
  Serial.print("\r");
  return cmmd + (char)((crc >> 8) & 0xFF) + (char)((crc >> 0) & 0xFF) + '\r';
}
